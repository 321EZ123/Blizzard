...
export default async function snowHandler(winterRequest, iceResponse) {
    ...
    try {
        ...
        if (!isJsAsset && frozenContentType.includes('text/html')) {
            ...
            if (targetDestination.includes('google.com/search')) {
                const formRegex = /<form\s+class="tsf"[^>]*role="search"[^>]*>[\s\S]*?<\/form>/i;

                frozenContent = frozenContent.replace(formRegex, '');
                ...
                `);
            } else if (targetDestination.includes('https://google.com')) {
                const filePath = path.join(process.cwd(), 'static', 'glacier', 'index.html');
                frozenContent = fs.readFileSync(filePath, 'utf8');
        ...
        return iceResponse.status(500).send(`<h1>Blizzard Storm Error</h1><p>${stormError.message}</p>`);
    }
}
